...
def main():
    ...
    pts_fts = tf.placeholder(tf.float32, shape=(batch_size, point_num, setting.data_dim), name='pts_fts')
    labels_seg = tf.placeholder(tf.int64, shape=(batch_size, point_num), name='labels_seg')
    labels_weights = tf.placeholder(tf.float32, shape=(batch_size, point_num), name='labels_weights')
    ...
    # net = model_builder.Net(points_augmented, features_augmented, is_training, setting)
    ...
    model_obj = model_builder.Model(tf.concat([points_augmented, features_augmented], -1),
                                    is_training,
                                    sample_num,
                                    NUM_LAYERS,
                                    NUM_NEIGHBORS,
