...
"""
This .py file reads the xyzrgb.npy from the folder. 
...
def main():
    ...
    for area_idx in range(2, 7):    # todo: 1, 7
        ...
        for room_idx, room in enumerate(rooms):
            room_marker = os.path.join(folder, room, ".room")
            if os.path.exists(room_marker):
                print('{}-{}/{} already processed, skipping'.format(datetime.now(), folder, room))
                continue
            filename_data = os.path.join(folder, room, 'xyzrgb.npy')
            print('{}-Loading {}...'.format(datetime.now(), filename_data))
            xyzrgb = np.load(filename_data)

            filename_labels = os.path.join(folder, room, 'label.npy')
            ...
            for offset_name, offset in offsets:
                ...
                # blocks is the splited blocks. point_block_indices is the index of block for each point
                ...
                print('{}-{} is split into {} blocks.'.format(datetime.now(), room, blocks.shape[0]))
                ...
                for block_idx in range(idx_last_non_empty_block + 1):
                    ...
                    for block_split_idx in range(block_split_num):
                        ...
                        label[idx_in_batch] = room_idx  # won't be used...
                        ...
                        if ((idx + 1) % batch_size == 0) or \
                            ...
                            if args.save_ply:
                                ...
                                filepath_label_ply = os.path.join(folder, room, 'ply_label',
                                ...
                                filepath_rgb_ply = os.path.join(folder, room, 'ply_rgb',
