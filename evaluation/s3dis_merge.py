...
def main():
    ...
    parser.add_argument('--datafolder', '-d', help='Path to evaluated area', default='/data/3D/s3dis_aligned/Area_5')
    args = parser.parse_args()
    print(args)

    room_list = os.listdir(args.datafolder)

    for room in room_list:
        output_path = os.path.join(args.datafolder, room, "pred.npy")
        label_length = np.load(os.path.join(args.datafolder, room, "label.npy")).shape[0]
        ...
        pred_list = [pred for pred in os.listdir(os.path.join(args.datafolder, room)) if
                     pred.split(".")[-1] == "h5" and "pred" in pred]
        for pred_file in pred_list:
            print(os.path.join(args.datafolder, room, pred_file))

            with h5py.File(os.path.join(args.datafolder, room, pred_file), 'r') as data:
        ...
        final_label[merged_confidence_zero >= merged_confidence_half] = merged_label_zero[
            merged_confidence_zero >= merged_confidence_half]
        final_label[merged_confidence_zero < merged_confidence_half] = merged_label_half[
            merged_confidence_zero < merged_confidence_half]
